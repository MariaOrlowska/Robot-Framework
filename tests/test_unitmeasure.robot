...
Min & Max UnitMeasure Length
    ...
    Connect To DB
    ${min_max}  Query  USE AdventureWorks2012; SELECT MIN(Name), MAX(Name) FROM [Production].[UnitMeasure];
...
Check for Specific UnitMeasure Values
    ...
    Connect To DB
    ${result}  Query  USE AdventureWorks2012; SELECT Name FROM [Production].[UnitMeasure] WHERE Name IN ('Each', 'Gallon', 'Meter');
